...
const CustomerDashboard = () => {
  ...
  useEffect(() => {
    const fetchBookedProperties = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/booked-properties`, {
          headers: {
  ...
  const checkRemainingDaysInitial = async (propertyId) => {
    try {
      ...
      const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/property-rent-timer/${propertyId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
