  ...
  useEffect(() => {
    ...
    try {
      ...
      const fetchUserData = async () => {
        try {
          const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/profileDetails/getUserProfile`, {
            headers: {
  ...
  const handleEditProfile = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_BASE_URL}/api/updateUser/updateProfile`,
        formDataToSend,
        {
          headers: {
            Authorization: `Bearer ${token}`,
